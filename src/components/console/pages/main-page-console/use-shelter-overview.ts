...
export type ShelterOverviewData = {
    operational_summary: {
        ...
        critical_capacity_halls: number;
    };
    capacity_summary: {
        total_capacity: number;
        occupied_capacity: number;
    ...
    requests_summary: {
        ...
        normal_priority_requests: number;
...
async function getShelterOverview(
    ...
    const url = `${baseUrl}/api/v1/admin/monitoring/shelter/overview${searchParams.toString() ? `?${searchParams.toString()}` : ""
        }`;
